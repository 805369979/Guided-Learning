...
class FerModel(object):
    ...
    def build_model(self):
        ...
        # x = mobilenet_block(x, filters=64, kernel_size=7, strides=2)
    ...
    def train(self):
        ...
        for key, v in enumerate(img_list_all):
            ...
            cv2.imwrite("kaggle{}".format(v), superimposed_img)
            plt.savefig("{}".format("kaggle" + str(uuid.uuid4()) + ".eps"))
        ...
        # 创建 CategoricalAccuracy 实例
        accuracy = tf.keras.metrics.CategoricalAccuracy()

        # 更新状态并计算准确率
        accuracy.update_state(y_test, yPred)
        result = accuracy.result().numpy()

        _val_recall = recall_score(y, yPred, average='micro')

        print("draw compliment" + str(_val_f1) + "*---" + str(_val_recall)+ "---"+str(result))
        ...
        for i in range(len(X_test)):
            ...
            temp = []
            y.append(x_real)
        with open("confidenceFile/kaggle.csv", "a", newline='') as f:
            writer = csv.writer(f)
...
def get_flops(model, input_shape,batch_size=32):
    """
    计算 Keras 模型的 GFLOPs（Giga Floating Point Operations）。
    要求 TensorFlow >= 2.0，兼容 2.5.0。

    Args:
        model: tf.keras.Model 实例
        input_shape: tuple, 不包含 batch 维度，例如 (224, 224, 3)

    Returns:
        flops: 总 FLOPs（浮点数）
        gflops: GFLOPs（FLOPs / 1e9）
    """
    # 1. 构造假输入
    if not isinstance(input_shape, (tuple, list)):
        raise ValueError("input_shape must be a tuple or list")

    real_input = tf.TensorSpec((batch_size,) + input_shape, tf.float32)

    # 2. 将 Keras 模型转为 ConcreteFunction
    full_model = tf.function(lambda x: model(x))
    concrete_func = full_model.get_concrete_function(real_input)

    # 3. 转换为常量图（冻结变量）
    frozen_func = convert_variables_to_constants_v2(concrete_func)
    frozen_graph = frozen_func.graph.as_graph_def()

    # 4. 使用 TF Profiler 计算 FLOPs
    run_meta = tf.compat.v1.RunMetadata()
    opts = tf.compat.v1.profiler.ProfileOptionBuilder.float_operation()
    # opts['output'] = 'none'  # 👈 关键：禁止输出到控制台
    # 只统计乘加操作（MACs），通常 1 MAC = 2 FLOPs，但 profiler 默认返回 FLOPs
